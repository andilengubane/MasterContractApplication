...
namespace MasterContractApplication.Application.Command.UserCommand
{
    ...
    public class RemoveUserCommandHandler(IUserRepository _userRepository) : IRequestHandler<RemoveUserCommand, bool>
    {
        public Task<bool> Handle(RemoveUserCommand request, CancellationToken cancellationToken)
        {
            return _userRepository.RemoveUserAsync(request.id);
        }
    }
}